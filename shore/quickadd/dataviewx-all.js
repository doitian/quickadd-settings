...
module.exports = async (params) => {
  ...
  const pages = api.pagePaths("#dataviewx");

  for (let page of pages) {
    const currentFile = vault.getAbstractFileByPath(page);
    ...
    console.log(`🔵info: dataview exported in ${currentFile.path}`);
    await vault.modify(currentFile, exported);
  }

  new Notice(`🔵info: dataview exported in ${pages.length} files`);
};
